...
class SemanticMapMaster:
    def __init__(self):
        ...
        while not (
            rospy.has_param("/depth_anything_ros_init")
            and rospy.has_param("/yolo_evsam_ros_init")
        ):
        ...
        self.color_to_process_pub = rospy.Publisher(
            "color_to_process", Image, queue_size=1
        )
        self.depth_to_process_pub = rospy.Publisher(
            "depth_to_process", Image, queue_size=1
        )
        ...
        try:
            with self.data_lock:
                ...
                if (not det_seg_processing) and (not depth_repair_processing):
                    ...
                    print(" ")
                    rospy.loginfo(
                        f"publish color_to_process and depth_to_process of timestamp:{time_stamp.to_sec()}"
                    )
                    self.latest_image = None
                    self.latest_depth = None
