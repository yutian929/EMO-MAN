...
from semantic_map_pkg.srv import Clip, ClipRequest, ClipResponse
...
class SemanticMapGenerator:
    def __init__(self):
        ...
        self.enable_new_objs_pub = rospy.get_param("~enable_new_objs_pub", True)
        ...
        ## 标注+掩码图像发布
        self.annotated_pub = rospy.Publisher("/semantic_annotated", Image, queue_size=1)
        self.masks_pub = rospy.Publisher("/semantic_masks", Image, queue_size=1)
        ## 语义对象det+seg服务, 客户端
        rospy.loginfo("Waiting for semantic object detection service...")
        ...
        rospy.loginfo("CLIP service is ready")
        ## 总循环定时器
    ...
    def extract_images_features(self, crops):
        """获取图像特征"""
        # 将图像转换为ROS消息
        ros_images = [
            self.bridge.cv2_to_imgmsg(crop, encoding="rgb8") for crop in crops
        ]

        # 调用CLIP服务
        try:
            response = self.clip_client(
                task="encode_images_from_ros_msg", images=ros_images
            )
            if response.success:
                rospy.loginfo("Images encoding successful")
                return np.array(response.array.data).reshape(
                    response.array.layout.dim[0].size,
                    response.array.layout.dim[1].size,
                )
            else:
                rospy.logerr(f"Images encoding failed: {response.message}")
                return None
        except rospy.ServiceException as e:
            rospy.logerr(f"CLIP service call failed: {str(e)}")
            return None
    ...
    def timer_callback(self, event):
        ...
        try:
            ...
            # 根据bbox对annotated图像进行裁剪，送入CLIP获取每个语义对象的图像特征
            crops = []
            for box in boxes:
                x1, y1, x2, y2 = (
                    int(box[0]),
                ...
                crops.append(crop)
            crops_features = self.extract_images_features(crops)
            ...
            if len(masks) > 0:
                ...
                # 生成当前帧语义对象
                for mask, label, score, crop_feature in zip(
                    masks, labels, scores, crops_features
                ):
                    semantic_obj = self.create_semantic_object(
                        ...
                        score=score,
                        annotated=annotated,
                        latest_image=image_snapshot,
                        latest_depth=depth_snapshot,
                        camera_info=camera_info_snapshot,
                        latest_transform=tf_snapshot,
                        crop_feature=crop_feature.tolist(),
                    )
                    if semantic_obj is not None:
                        if self.enable_new_objs_pub:
                            # 发布语义对象
                            self.semantic_object_pub.publish(semantic_obj)
                            rospy.loginfo(
                                f"Published {semantic_obj.category} with feature[:5] = {crop_feature[:5]} "
                            )
                        else:
                            rospy.logwarn(
                                f"Filtered {semantic_obj.category} with feature[:5] = {crop_feature[:5]} "
                            )
