...
class SemanticMapGenerator:
    def __init__(self):
        ...
        self.camera_info_sub = rospy.Subscriber(
            topic_camera_info_sub, CameraInfo, self.read_camera_info, queue_size=2
        )
        ...
        ts1.registerCallback(
            lambda color_image_msg, depth_raw_msg, tf_msg: self.executor.submit(
                self.store_image, color_image_msg, depth_raw_msg, tf_msg
            )
        )

        # 订阅图像标注信息
        # self.annotation_info_sub = rospy.Subscriber("annotation_info", AnnotationInfo, self.annotate, queue_size=2)
        self.annotation_info_sub = rospy.Subscriber(
            "annotation_info",
            AnnotationInfo,
            lambda msg: self.executor.submit(self.annotate, msg),
            queue_size=2,
        )
        ...
        rospy.loginfo(
            "semantic_map_generator_distributed_node initialization complete."
        )
    ...
    def sync_sub_callback(self, depth_repaired_msg, mask_info_msg):
        ...
        if transform is None:
            rospy.logwarn(
                f"tf transform for timestamp {time_stamp.to_sec()} not found."
            )
            return  # tf变换不存在，直接返回
        ...
        # 获取修复后的深度
        depth_repaired = self.bridge.imgmsg_to_cv2(
            depth_repaired_msg, desired_encoding="passthrough"
        )
        # 获取掩码
        masks_stacked = self.bridge.imgmsg_to_cv2(
            mask_info_msg.segmasks, desired_encoding="passthrough"
        )  # 保持原始格式
        ...
        for mask, label, score in zip(masks, labels, scores):
            ...
            second_depth_repaired_masked = self.second_depth_repair(
                depth_raw.copy(), depth_repaired.copy(), mask
            )
        ...
        rospy.loginfo(
            f"create semantic map time: {depth_repair_time:.1f} ms for timestamp:{time_stamp.to_sec()}"
        )
        print(" ")
    ...
    def create_semantic_object(
        ...
        # 进行坐标变换，得到世界坐标
        world_coordinates = self.pixel_to_world_batch(
            x, y, z, camera_info, transform_matrix
        )
        ...
        if self.downsample_step > 1:  # 只有步长大于1才进行下采样
            sampled_indices = np.arange(
                0, world_coordinates.shape[1], self.downsample_step
            )  # 统一索引
    ...
    def pixel_to_world_batch(
        self, u_coords, v_coords, z_coords, camera_info, transform_matrix
    ):
        """
    ...
    def second_depth_repair(
        self, depth_raw, depth_repaired, mask, threshold=500, min_valid_points=50
    ):
        # 1. 使用掩码选择有效区域的深度
        valid_mask = mask > 0  # 掩码部分，确保有效区域
        ...
        mask_depth_repaired = (
            depth_repaired[valid_mask].copy().astype(np.float32)
        )  # 选择所有掩码为非零部分的修复深度
        ...
        if len(valid_depth_raw) >= min_valid_points:
            ...
            ).T  # X_valid 和常数项（1）构成设计矩阵
            # 求解最小二乘法
            params, residuals, rank, s = np.linalg.lstsq(X_stack, D_valid, rcond=None)
            # 从返回的参数中获取拟合参数 A 和 b
            A, b = params
            fitted_depth = (
                A * (mask_depth_repaired) + b
            )  # 仅掩码部分的深度，拼接（x,y,z）数组，用于后续点云计算
        else:
    ...
    def annotate(self, annotation_info_msg):
        ...
        boxes = np.array(annotation_info_msg.boxes.data).reshape(
            (
                annotation_info_msg.boxes.layout.dim[0].size,
                annotation_info_msg.boxes.layout.dim[0].stride,
            )
        )
        ...
        annotated_image = box_annotator.annotate(
            scene=self.processing_image.copy(), detections=detections
        )
